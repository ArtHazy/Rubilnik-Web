...
export const QuizTile = ({quiz, ind, upd, self, quizzes}) => {
    ...
    return <div className='QuizTile'>
        <button id="del" onClick={()=>{Array.isArray(quizzes) && confirm("delete?")? (console.log('deleted: ',quizzes.splice(ind,1)) , upd()) : null}}>delete</button>
        <button id="edit" onClick={()=>{console.log(quiz), window.location='/edit-quiz/'+ind}}> {quiz.title}</button>
        <button id="run" onClick={()=>startRoomAsHost(self.id, quiz, ind, navigate)}>play</button>
    </div>
}
